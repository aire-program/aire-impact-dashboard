...
st.plotly_chart(plot_registrations_vs_attendance(data["workshops"]), use_container_width=True)

st.subheader("Engagement Funnel")
# Simple funnel metrics
total_registrations = data["workshops"]["registrations"].sum()
total_attended = data["workshops"]["attended"].sum()
total_adoption_events = len(data["adoption_events"])
